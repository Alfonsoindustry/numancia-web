...
export default function Navbar() {
    ...
    return (
        <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled || isMobileMenuOpen ? "glass border-b border-white/5 py-4" : "bg-transparent py-6"}`}>
            ...
            <div
                className="absolute bottom-0 left-0 h-[2px] bg-neon-orange transition-[width] duration-75 ease-out pointer-events-none"
                ...
                <div className="hidden md:flex items-center gap-8 text-sm font-medium text-text-slate">
                    {navLinks.map((link) => (
                        <Link
                            key={link.href}
                            href={link.href}
                            className={`relative hover:text-neon-orange transition-colors py-2 ${isActive(link.href) ? "text-neon-orange" : ""}`}
                        >
                            {link.label}
                            {isActive(link.href) && (
                                <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-neon-orange rounded-full" />
                            )}
                        </Link>
                    ))}
                    <Link
                        href="/contacto"
                        className={`relative hover:text-neon-orange transition-colors py-2 ${isActive("/contacto") ? "text-neon-orange" : ""}`}
                    >
                        Contacto
                        {isActive("/contacto") && (
                            <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-neon-orange rounded-full" />
                ...
                <AnimatePresence>
                    {isMobileMenuOpen && (
                        <motion.div
                            ...
                            className="fixed inset-0 bg-[#0A0A0A]/95 backdrop-blur-xl pt-32 px-6 flex flex-col gap-8 md:hidden h-screen"
                        >
                            <div className="flex flex-col gap-6 items-center">
                                {[...navLinks, { href: "/contacto", label: "Contacto" }].map((link) => (
                                    <Link
                                        key={link.href}
                                        href={link.href}
                                        className={`text-2xl font-bold hover:text-neon-orange transition-colors ${isActive(link.href) ? "text-neon-orange" : ""}`}
